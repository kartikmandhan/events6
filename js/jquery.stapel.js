    ...
    _setInitialStyle: function () {
      for (var pile in this.piles) {
        ...
        for (var i = 0, len = p.elements.length; i < len; ++i) {
          ...
          if (i === len - 2) {
            styleCSS = {
              transform: "rotate(" + this.options.pileAngles + "deg)",
            };
          } else if (i === len - 3) {
            styleCSS = {
              transform: "rotate(-" + this.options.pileAngles + "deg)",
      ...
      for (var pile in this.piles) {
        ...
        for (var i = 0, len = p.elements.length; i < len; ++i) {
              ...
              pile === this.pileName
                ? {
                    ...
                    transition: this.support
                      ? "left " +
                        this.options.pileAnimation.openSpeed +
                        "ms " +
                        (len - i - 1) * this.options.delay +
                        "ms " +
                        this.options.pileAnimation.openEasing +
                        ", top " +
                        this.options.pileAnimation.openSpeed +
                        "ms " +
                        (len - i - 1) * this.options.delay +
          ...
          if (pile === this.pileName) {
            ...
            fs.transform =
              this.options.randomAngle && i !== p.index
                ? "rotate(" +
              ...
              styleCSS =
                pile === this.pileName
                  ? {
                      transition: this.support
                        ? "left " +
                          this.options.pileAnimation.closeSpeed +
                          "ms " +
                          this.options.pileAnimation.closeEasing +
                          ", top " +
                          this.options.pileAnimation.closeSpeed +
                          "ms " +
                          this.options.pileAnimation.closeEasing +
  ...
  $.fn.stapel = function (options) {
    ...
    if (typeof options === "string") {
      ...
      this.each(function () {
        if (!instance) {
          logError(
            "cannot call methods on stapel prior to initialization; " +
